...
resource "github_actions_variable" "app_sa_email" {
  repository    = var.repository_name
  variable_name = "APP_SA_EMAIL"
  value         = google_service_account.app_sa["dev"].email
  depends_on    = [github_repository.repo]
}

resource "github_actions_variable" "logs_bucket_name" {
  repository    = var.repository_name
  variable_name = "LOGS_BUCKET_NAME"
  value         = google_storage_bucket.logs_data_bucket[var.dev_project_id].name
  depends_on    = [github_repository.repo]
}
